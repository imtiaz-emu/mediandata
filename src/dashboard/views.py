...
def update(request, id=None):
    dashboard = get_object_or_404(Dashboard, id=id)
    selected_wbs = json.loads(request.POST.get('workboards', None))
    db_workboards = dashboard.workboards
    db_wb_ids = db_workboards.all().values_list('id', flat=True)
    try:
        # delete previous workboards
        for db_wb in db_wb_ids:
            workboard = get_object_or_404(Workboard, id=db_wb)
            if workboard in db_workboards.all():
                db_workboards.remove(workboard)
        # add newly selected workboards
        for s_wb in selected_wbs:
            workboard = get_object_or_404(Workboard, id=s_wb)
            if workboard not in db_workboards.all():
                db_workboards.add(workboard)
